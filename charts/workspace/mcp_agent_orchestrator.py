...
def _assistant_command(assistant: str) -> str:
    """Resolve an assistant ID to a CLI command string."""
    cmd_map = {
        'claude': 'claude',
        'ante': 'ante',
    }
    ...
    return cmd_map.get(assistant, 'claude')
...
def _tool_spawn_agent(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cli_cmd = _assistant_command(assistant)
    ...
    # Paste the prompt after a brief init delay
    def _send_prompt():
        time.sleep(3)
        try:
...
if __name__ == '__main__':
    # threading is only used for the prompt-delivery background task
    import threading  # noqa: E402 — lazy import to avoid overhead on non-spawn paths
    sys.exit(main())